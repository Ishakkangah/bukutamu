    ...
    var Webcam = {
        ...
        getMovie: function () {
            if (!this.loaded)
                return this.dispatch(
                    "error",
                    new a("Flash Movie is not loaded yet")
                );
            var e = document.getElementById("webcam_movie_obj");
            if (!e || !e._snap)
                e = document.getElementById("webcam_movie_embed");
            if (!e)
                this.dispatch(
                    "error",
                    new a("Cannot locate Flash movie in DOM")
                );
            return e;
        },
        freeze: function () {
        ...
        snap: function (e, t) {
            ...
            if (!this.loaded)
                return this.dispatch(
                    "error",
                    new i("Webcam is not loaded yet")
                );
            if (!e)
                return this.dispatch(
                    "error",
            ...
            if (this.userMedia) {
                ...
                h.onload = n;
                h.src =
                    "data:image/" + this.params.image_format + ";base64," + d;
            }
            return null;
        },
