...
<div class="main-content">
    ...
    <section>
        <div class="px-4 section-header  bg-white py-2 rounded">
            ...
            @if (Auth::user())
            <form class="col-12 d-flex justify-content-between  mx-0 px-0" action="{{ asset('cari') }}" method="get">
                <input type="text" class="form-control" name="cari" placeholder="Pencarian berdasarkan nama..">
                        ...
                        <tr>
                            ...
                            <td class="align-middle">
                                <div class="d-flex justify-content-center">
                                    {{-- details tamu --}}
                                    <a href="{{ route('detailsTamu', $bukutamu->id) }}" class="bg-info px-2 py-2 rounded text-white"><i class="bi bi-eye-fill text-white"></i></a>
                                    {{-- edit tamu --}}
                                    <a href="{{ route('edit', $bukutamu->id) }}" class="bg-primary px-2 py-2 rounded text-white mx-2"><i class="bi bi-pencil-fill text-white"></i></a>

                                    {{-- hapus --}}
                                    <form action="{{ route('delete', $bukutamu->id) }}" method="post" class="">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" class="bg-danger px-2 py-2 rounded text-white border-0" onclick="return confirm('Yakin ?')">
                            ...
                            <td class="text-center">
                                ...
                                <a href="{{ route('detailsTamu', $bukutamu->id) }}" class="text-decoration-none text-muted">
                                    <img src="{{ $bukutamu->takeImage ? $bukutamu->takeImage : asset('img/profile.jpg') }}" class="rounded" alt="photo" width="40px">
                                </a>
                                @endif
                            </td>
                            <td class="text-center align-middle"> <a href="{{ route('detailsTamu', $bukutamu->id) }}" class="text-decoration-none text-muted">{{ strlen($bukutamu->name) > 10 ? substr($bukutamu->name, 0, 20) . '...' : strtoUpper($bukutamu->name) }}</a>
                            </td>
                            <td class="text-center text-muted align-middle"> <a href="{{ route('detailsTamu', $bukutamu->id) }}" class="text-decoration-none text-muted">
    ...
    <div class="col colum_kiri">
        <div class="wrapper">
            ...
            <form action="{{ route('store') }}" method="post" enctype="multipart/form-data" class="" autocomplete="off">
                ...
                <div class="mb-3">
                    <input type="text" class="form-control" name="name" id="name" placeholder="Nama" required value="{{ old('name') }}">
                    @error('name')
                ...
                <div class="mb-3">
                    <input type="text" class="form-control" name="instansi" id="instansi" required placeholder="Instansi asal" value="{{ old('instansi') }}">
                    @error('instansi')
                    <small class="text-danger">
                        <div class="bg-danger rounded text-white mt-1 px-2 py-2">{{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    <input type="text" class="form-control" id="perihal" name="perihal" required placeholder="Perihal" value="{{ old('perihal') }}">
                    @error('perihal')
                    <small class="text-danger">
                        <div class="bg-danger rounded text-white mt-1 px-2 py-2">{{ $message }}
                        </div>
                ...
                <div class="mb-3">
                    <input type="text" class="form-control" id="pekerjaan" name="pekerjaan" required placeholder="perkerjaan" value="{{ old('pekerjaan') }}">
                    @error('pekerjaan')
                    <small class="text-danger">
                        <div class="bg-danger rounded text-white mt-1 px-2 py-2">{{ $message }}
                ...
                <div class="mb-3">
                    <input type="text" class="form-control" id="keterangan" name="keterangan" placeholder="Nomor HP yang dapat dihubungi" style=" max-width:100%;min-height:50px;height:100%;width:100%;" value="{{ old('keterangan') }}" required>
                    @error('keterangan')
                    <small class="text-danger">
                        <div class="bg-danger rounded text-white mt-1 px-2 py-2">{{ $message }}
                        </div>
